...
namespace EmptyRoomAlert.WebApi.Codes.Core.Factories.Aggregates
{
    ...
    {
        RoomStateResponseModel Create(RoomState roomState);
        ICollection<RoomStateResponseModel> Create(ICollection<RoomState> roomStates);
        ResponseCollectionModel<RoomStateResponseModel> Create(ICollection<RoomState> roomStates, Pagination pagination, SortBy sortBy, int total);
    }
}
